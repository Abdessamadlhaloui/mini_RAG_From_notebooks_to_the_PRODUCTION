...
class TestInputValidation:
    ...
    def test_query_too_long_returns_422(self, client, auth_headers):
        long_query = 'a' * 2001
        response = client.post('/api/v1/query', json={'query': long_query}, headers=auth_headers)
        assert response.status_code == 422
    def test_top_k_out_of_range_returns_422(self, client, auth_headers):
        response = client.post('/api/v1/query', json={'query': 'valid question', 'top_k': 100}, headers=auth_headers)
        assert response.status_code == 422
class TestIngestValidation:
    def test_ingest_unsupported_format_returns_400(self, client, auth_headers):
        files = {'file': ('test.csv', b'col1,col2', 'text/csv')}
        response = client.post('/api/v1/ingest', files=files, headers=auth_headers)
